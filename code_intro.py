...
# 2. Configurar la pantalla COMPLETA
...
try:
    ...
    try:
        ...
        if os.path.exists(gif_path):
            ...
            print(f"✅ explosion.gif cargada correctamente - {gif.n_frames} frames")
        elif os.path.exists(assets_gif_path):
            gif = Image.open(assets_gif_path)
            print(f"✅ assets/explosion.gif cargada correctamente - {gif.n_frames} frames")
        else:
        ...
        print(f"✅ {len(imagenes_explosion)} frames de explosión cargados")
        usar_gif = True
        
    except ImportError:
        print("⚠️ PIL no está instalado - usando explosión procedural")
        usar_gif = False
    except Exception as e:
        print(f"⚠️ Error cargando GIF: {e} - usando explosión procedural")
        usar_gif = False
    
    # Crear explosión procedural como fallback
    if not usar_gif or not imagenes_explosion:
        ...
        print("✅ Explosión procedural creada")
    
    # Verificar que las imágenes principales se cargaron
    if imagen_muro and imagen_suelo and imagen_jugador and imagen_cazador:
        usar_imagenes = True
        print("🎉 Todas las imágenes principales cargadas - Modo Imágenes Activado")
    else:
        raise Exception("Una o más imágenes principales no se cargaron correctamente")
        
except Exception as e:
    print(f"❌ Error cargando imágenes: {e}")
    print("🔧 Usando modo colores simples")
    usar_imagenes = False
    imagen_jugador = None
    imagen_cazador = None
    imagen_mina_original = None
...
# 9. Clase Mina/Trampa - CORREGIDO: Ahora almacena la imagen escalada
class Mina:
    def __init__(self, x, y, tamaño_celda, imagen_mina_escalada=None):
        self.x = x
        self.y = y
        self.tamaño = tamaño_celda // 2  # Más pequeña - la mitad del tamaño del jugador
        self.activa = True
        ...
        self.imagen_mina_escalada = imagen_mina_escalada  # Guardar la imagen escalada
    ...
    def dibujar(self, superficie, tamaño_celda):
        if self.explotando:
            ...
            if self.frame_explosion < len(imagenes_explosion):
                ...
                # Escalar la explosión según el radio
                tamaño_explosion = self.radio_explosion * 2
                explosion_escalada = pygame.transform.scale(explosion_img, 
                                                          (tamaño_explosion, tamaño_explosion))
                superficie.blit(explosion_escalada, 
                              (self.x - tamaño_explosion // 2, self.y - tamaño_explosion // 2))
        elif self.activa:
            # Dibujar mina activa - CORREGIDO: Usar la imagen almacenada
            if usar_imagenes and self.imagen_mina_escalada:
                # Calcular posición correcta (centro de la mina)
                mina_x = self.x - self.tamaño // 2
                mina_y = self.y - self.tamaño // 2
                superficie.blit(self.imagen_mina_escalada, (mina_x, mina_y))
            else:
                # Representación gráfica de fallback
    ...
    def actualizar(self):
        # Desactivar inmunidad después del tiempo de activación
        ...
        if self.explotando:
            ...
            # Terminar explosión después de mostrar todos los frames
            if self.frame_explosion >= len(imagenes_explosion):
                return False  # Eliminar mina
        return True  # Mantener mina
    ...
    def esta_en_rango_explosion(self, x, y, radio_objeto, es_jugador=False):
        ...
        # Si es el jugador y la mina todavía tiene inmunidad, no le hace daño
        if es_jugador and self.jugador_inmune:
            return False
            
        # Solo verificar durante los primeros frames de la explosión
...
# 10. Clase Cazador (se mantiene igual)
class Cazador:
    ...
    def mover_hacia_objetivo(self, objetivo_x, objetivo_y, mapa, tamaño_celda):
        ...
        if (not self.camino or self.contador_pasos >= 20 or 
            distancia_objetivo > tamaño_celda * 3):
            
            self.camino = self.encontrar_camino_corto(
                (self.x, self.y), (objetivo_x, objetivo_y), mapa, tamaño_celda
            )
            self.contador_pasos = 0
        ...
        if self.camino:
            ...
            if distancia > 0:
                ...
                if (celda_actual_x == siguiente_celda[1] and 
                    celda_actual_y == siguiente_celda[0]):
                    self.camino.pop(0)
        ...
        if distancia < tamaño_celda * 5:
            ...
            if self.modo_persecucion and tiempo_desde_deteccion < 3000:
                self.mover_hacia_objetivo(
                    self.ultima_posicion_jugador[0], 
                    self.ultima_posicion_jugador[1], 
                    mapa, tamaño_celda
                )
            else:
                self.modo_persecucion = False
...
# 11. Crear botones del menú
ancho_boton = 300
alto_boton = 60
espacio_botones = 20
centro_x = ANCHO // 2

boton_escapar = Boton(centro_x - ancho_boton//2, 250, ancho_boton, alto_boton, 
                      "🔓 MODO ESCAPAR", AZUL, AZUL_HOVER)
boton_cazar = Boton(centro_x - ancho_boton//2, 250 + alto_boton + espacio_botones, 
                   ancho_boton, alto_boton, "🎯 MODO CAZADOR", VERDE, (0, 200, 0))
boton_puntuaciones = Boton(centro_x - ancho_boton//2, 250 + (alto_boton + espacio_botones) * 2, 
                          ancho_boton, alto_boton, "🏆 PUNTUACIONES", GRIS_OSCURO, GRIS)
boton_salir = Boton(centro_x - ancho_boton//2, 250 + (alto_boton + espacio_botones) * 3, 
                   ancho_boton, alto_boton, "❌ SALIR", ROJO, (200, 0, 0))

# 12. Campo de texto para registro
campo_nombre_rect = pygame.Rect(centro_x - 200, 300, 400, 50)
campo_nombre_activo = False
texto_nombre = ""

# 13. Generador de laberintos
...
# 14. Función para dibujar el menú principal
...
# 15. Función para dibujar pantalla de registro
def dibujar_pantalla_registro():
    ...
    if campo_nombre_activo and pygame.time.get_ticks() % 1000 < 500:
        ...
        pygame.draw.line(pantalla, BLANCO, (cursor_x, campo_nombre_rect.y + 10), 
                        (cursor_x, campo_nombre_rect.y + campo_nombre_rect.height - 10), 2)
...
# 16. Función para dibujar pantalla de puntuaciones
def dibujar_pantalla_puntuaciones():
    ...
    puntuaciones_ordenadas = sorted(puntuaciones, key=lambda x: x["puntos"], reverse=True)
    
    # Mostrar las top 10 puntuaciones
    y_pos = 180
    for i, puntuacion in enumerate(puntuaciones_ordenadas[:10]):
        texto = f"{i+1}. {puntuacion['nombre']}: {puntuacion['puntos']} pts - {puntuacion['modo']}"
        texto_surf = fuente_texto.render(texto, True, BLANCO)
        pantalla.blit(texto_surf, (ANCHO//2 - texto_surf.get_width()//2, y_pos))
        y_pos += 40
    
    # Si no hay puntuaciones
    if not puntuaciones:
        texto_vacio = fuente_texto.render("No hay puntuaciones registradas", True, BLANCO)
...
# 17. Función para guardar puntuación
    ...
    try:
        with open("puntuaciones.txt", "w") as f:
            for punt in puntuaciones:
                f.write(f"{punt['nombre']},{punt['puntos']},{punt['modo']},{punt['fecha']}\n")
    except:
        print("⚠️ No se pudo guardar las puntuaciones en archivo")

# 18. Función para el juego principal CON SISTEMA DE PUNTUACIONES
def jugar_modo(jugador_color, modo_nombre, generar_nuevo_mapa=True):
    global estado_actual, mapa, puntuaciones
    ...
    # CORREGIDO: Escalar la imagen de la mina correctamente
    if usar_imagenes:
        ...
        # CORREGIDO: Escalar la mina con el tamaño correcto
        mina_tamaño = tamaño_celda // 2  # Tamaño más pequeño
        if imagen_mina_original:
            imagen_mina_escalada = pygame.transform.scale(imagen_mina_original, (mina_tamaño, mina_tamaño))
            print(f"✅ Mina escalada a {mina_tamaño}x{mina_tamaño} píxeles")
        else:
            imagen_mina_escalada = None
            print("⚠️ No hay imagen de mina disponible")
    ...
    tiempo_limite_base = 15000  # 15 segundos para puntuación máxima
    ...
    def hay_colision(x, y, es_jugador=True):
        ...
        for px, py in puntos_verificacion:
            ...
            if (celda_y < 0 or celda_y >= len(mapa) or 
                celda_x < 0 or celda_x >= len(mapa[0])):
                return True
    ...
    def colocar_mina():
        ...
        if (minas_activas < max_minas and 
            tiempo_actual - tiempo_ultima_mina > cooldown_mina):
            
            # CORREGIDO: Pasar la imagen escalada al crear la mina
            nueva_mina = Mina(jugador_x, jugador_y, tamaño_celda, imagen_mina_escalada)
            minas.append(nueva_mina)
            print(f"💣 Mina colocada en posición ({int(jugador_x)}, {int(jugador_y)})! (Jugador inmune por 0.5 segundos)")
            return True
        return False
    
    def activar_minas_cercanas():
        """Activar minas cuando cazadores pasen cerca - EL JUGADOR NO ACTIVA MINAS"""
        minas_activadas = 0
        
        for mina in minas:
            if mina.activa and not mina.explotando:
                # SOLO verificar cazadores, NO el jugador
                for cazador in cazadores:
                    if cazador.vivo:
    ...
    def verificar_explosiones():
        """Verificar explosiones - EL JUGADOR ES INMUNE"""
        ...
        for mina in minas[:]:
            ...
            if mina.explotando:
                # EL JUGADOR ES INMUNE - no verificar colisión con jugador
                # Solo verificar cazadores
                for cazador in cazadores:
    ...
    def calcular_puntuacion(tiempo_transcurrido):
        """Calcula la puntuación basada en el tiempo y cazadores eliminados"""
        tiempo_segundos = tiempo_transcurrido // 1000
        
        # Puntuación base si se completa en menos de 15 segundos
        if tiempo_segundos <= 15:
            puntos_tiempo = puntuacion_base
        else:
            # Penalización después de 15 segundos
            segundos_extra = tiempo_segundos - 15
            puntos_tiempo = max(0, puntuacion_base - (segundos_extra * penalizacion_por_segundo))
        
        # Bonus por cazadores eliminados
    ...
    while ejecutando_juego:
        ...
        if not juego_terminado:
            ...
            # Verificar colisión con cazadores
            ...
            if verificar_salida():
                ...
                puntuacion_final = calcular_puntuacion(tiempo_transcurrido)
                # Guardar puntuación
        ...
        salida_rect = pygame.Rect(
            (len(mapa[0]) - 2) * tamaño_celda, 
            (len(mapa) - 2) * tamaño_celda, 
            tamaño_celda, tamaño_celda
        )
        pygame.draw.rect(pantalla, VERDE, salida_rect)
        
        # CORREGIDO: Dibujar minas - ahora cada mina tiene su propia imagen
        for mina in minas:
            mina.dibujar(pantalla, tamaño_celda)  # No necesitamos pasar la imagen aquí
        ...
        # Información de puntuación en tiempo real
        if not juego_terminado:
        ...
        # Información de terreno para modo cazador
...
# 19. Cargar puntuaciones al inicio
                ...
                for linea in f:
                    ...
                    if len(partes) >= 3:
                        puntuaciones.append({
                            ...
                            "puntos": int(partes[1]),
                            "modo": partes[2],
                            "fecha": partes[3] if len(partes) > 3 else "Desconocida"
                        })
            print(f"✅ {len(puntuaciones)} puntuaciones cargadas")
    except:
        print("⚠️ No se pudieron cargar las puntuaciones anteriores")

# Cargar puntuaciones al inicio
cargar_puntuaciones()

# 20. Bucle principal del juego
ejecutando = True
while ejecutando:
